<?php
	...
	if(!empty($action) && ($action == 'add' || $action == 'edit' )){
		...
		<input size="10" name="prix_visu" value="'.price($prix).'"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ('.$langs->trans('BuyingPrice').' : <span size="10" name="prix_fourn_visu">'.$prixAchat.'</span>)</td></tr>';
	...
	if(!empty($conf->global->TARIF_LOG_TARIF_UPDATE)) {
		...
		$sql = strtr($sql, array('tarif_conditionnement'=>'tarif_conditionnement_log', 'AS date_fin'=>'AS date_fin, tc.motif_changement')); // Même requête mais dans la table log

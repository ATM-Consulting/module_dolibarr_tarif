...
//			var_dump($action,$object);
		...
		elseif(($action == 'LINEORDER_UPDATE' || $action == 'LINEPROPAL_UPDATE' || $action == 'LINEBILL_UPDATE'  || ($action==='LINEORDER_SUPPLIER_UPDATE' && $useWithSupplierOrders))
			...
			if((isset($object->oldline->qty) && $object->oldline->qty != $object->qty) || (floatval($res->tarif_poids * pow(10, $res->poids)) != floatval($poids * pow(10, $weight_units)) && !$conf->global->TARIF_DONT_ADD_UNIT_SELECT)){
